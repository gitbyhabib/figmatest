<template>
  <div class="nft-market-place">
    <div class="top-menu">
      <b class="nfters"> <a href="#">NFters</a> </b>
      <div class="connect-wallet-wrapper">
        <b class="connect-wallet"> <button>Connect Wallet</button></b>
      </div>
      <div class="upload-wrapper">
        <b class="connect-wallet"><button class="text-white">Upload</button></b>
      </div>
      <div class="marketplace-parent">
        <div class="marketplace"><a href="#">Marketplace</a></div>
        <div class="resource"><a href="#">Resource</a></div>
        <div class="about"><a href="#">About</a></div>
      </div>
      <div class="top-menu-child"></div>
      <div class="top-menu-item"></div>
      <div class="top-menu-inner"></div>
      <input type="search" placeholder="Search" class="search" />
      <img
        class="search-normal-1-1-icon"
        alt=""
        src="../assets/image/searchnormal1-1.svg"
      />
    </div>
    <div class="hero">
      <img class="dot-icon" alt="" src="../assets/image/dot.svg" />
      <div class="explore-now-wrapper">
        <div class="explore-now">
          <button class="text-white">Explore Now</button>
        </div>
      </div>
      <div class="discover-and-collect-digital">
        Discover, and collect Digital Art NFTs
      </div>
      <div class="digital-marketplace-for-crypto">
        Digital marketplace for crypto collectibles and non-fungible tokens
        (NFTs). Buy, Sell, and discover exclusive digital assets.
      </div>
      <div class="k-parent">
        <div class="k"><b>98</b><span>k+</span></div>
        <div class="artwork">Artwork</div>
      </div>
      <div class="k-group">
        <div class="k"><b>12</b><span>k+</span></div>
        <div class="artwork">Auction</div>
      </div>
      <div class="k-container">
        <div class="k"><b>15</b><span>k+</span></div>
        <div class="artwork">Artist</div>
      </div>
      <img
        class="unsplashtzcrfpsniiq-icon"
        alt=""
        src="../assets/image/unsplashtzcrfpsniiq@2x.png"
      /><img
        class="unsplashpvoepplw818-icon"
        alt=""
        src="../assets/image/unsplashpvoepplw818@2x.png"
      /><img
        class="unsplashe8ufcyxz514-icon"
        alt=""
        src="../assets/image/unsplashe8ufcyxz514@2x.png"
      />
      <div class="rectangle-parent">
        <div class="group-child"></div>
        <div class="eth-parent">
          <b class="eth">0.25 ETH</b>
          <div class="h-43m-42s">
            <b>12</b><span>h </span><b>43</b><span>m </span><b>42</b
            ><span>s</span>
          </div>
          <div class="current-bid">Current Bid</div>
          <div class="ends-in">Ends in</div>
          <img
            class="ethereum-2-icon"
            alt=""
            src="../assets/image/ethereum-2.svg"
          />
        </div>
      </div>
      <div class="arkhan17-parent">
        <b class="arkhan17">Arkhan17</b
        ><img
          class="group-item"
          alt=""
          src="../assets/image/ellipse-95@2x.png"
        />
      </div>
      <b class="abstr-gradient-nft">Abstr Gradient NFT</b>
      <div class="ellipse-parent">
        <img class="group-inner" alt="" src="../assets/image/ellipse-4.svg" />
        <div class="l-parent">
          <div class="l">L</div>
          <div class="i">I</div>
          <div class="v">V</div>
          <div class="e">E</div>
          <div class="text"></div>
          <div class="a">A</div>
          <div class="u">U</div>
          <div class="c">C</div>
          <div class="t">T</div>
          <div class="i1">I</div>
          <div class="o">O</div>
          <div class="n">N</div>
          <div class="text1"></div>
          <div class="div">•</div>
          <div class="text2"></div>
          <div class="l1">L</div>
          <div class="i2">I</div>
          <div class="v1">V</div>
          <div class="e1">E</div>
          <div class="text3"></div>
          <div class="a1">A</div>
          <div class="u1">U</div>
          <div class="c1">C</div>
          <div class="t1">T</div>
          <div class="i3">I</div>
          <div class="o1">O</div>
          <div class="n1">N</div>
          <div class="text4"></div>
          <div class="div1">•</div>
          <div class="text5"></div>
        </div>
        <img
          class="ethereum-2-icon1"
          alt=""
          src="../assets/image/ethereum-21.svg"
        />
      </div>
    </div>
    <div class="amaze-nft">
      <div class="amaze-nft-child"></div>
      <b class="the-amazing-nft-art-of-the-wor"
        >The amazing NFT art of the world here</b
      >
      <div class="chart-square-2-parent">
        <img
          class="chart-square-2-icon"
          alt=""
          src="../assets/image/chartsquare-2.svg"
        /><b class="growth-transaction">Growth Transaction</b>
        <div class="lorem-ipsum-dolor-sit-amet-co">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam etiam
          viverra tellus
        </div>
      </div>
      <div class="card-tick-1-1-parent">
        <img
          class="chart-square-2-icon"
          alt=""
          src="../assets/image/cardtick1-1.svg"
        /><b class="growth-transaction">Fast Transaction</b>
        <div class="lorem-ipsum-dolor-sit-amet-co1">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam etiam
          viverra tellus imperdiet.
        </div>
      </div>
    </div>
    <div class="collection-over">
      <img
        class="unsplashf56y7dgrakc-icon"
        alt=""
        src="../assets/image/unsplashf56y7dgrakc@2x.png"
      />
      <div class="the-futr-abstr-parent">
        <b class="the-futr-abstr">The Futr Abstr</b>
        <div class="in-the-stock">10 in the stock</div>
        <img
          class="ellipse-icon"
          alt=""
          src="../assets/image/ellipse-951@2x.png"
        />
        <div class="eth-group">
          <b class="eth">0.25 ETH</b>
          <div class="current-bid">Highest Bid</div>
          <img
            class="ethereum-2-icon"
            alt=""
            src="../assets/image/ethereum-22.svg"
          />
        </div>
      </div>
      <div class="group-parent">
        <div class="unsplash8uzpyniu-rq-parent">
          <img
            class="unsplash8uzpyniu-rq-icon"
            alt=""
            src="../assets/image/unsplash8uzpyniurq@2x.png"
          />
          <div class="arkhan">arkhan</div>
        </div>
        <div class="the-futr-abstr-group">
          <b class="the-futr-abstr1">The Futr Abstr</b>
          <div class="of-8">1 of 8</div>
          <img
            class="group-child1"
            alt=""
            src="../assets/image/ellipse-952@2x.png"
          />
          <div class="ethereum-2-parent">
            <img
              class="ethereum-2-icon3"
              alt=""
              src="../assets/image/ethereum-23.svg"
            /><b class="eth2">0.25 ETH</b>
          </div>
          <div class="cta-place-a-bid">
            <div class="place-a-bid">
              <button class="text-white">Place a bid</button>
            </div>
          </div>
        </div>
      </div>
      <div class="group-container">
        <div class="unsplash8uzpyniu-rq-parent">
          <img
            class="unsplash8uzpyniu-rq-icon"
            alt=""
            src="../assets/image/unsplashtyg0rvhotre@2x.png"
          />
          <div class="arkhan">arkhan</div>
        </div>
        <div class="the-futr-abstr-group">
          <b class="the-futr-abstr1">The Futr Abstr</b>
          <div class="of-8">1 of 8</div>
          <img
            class="group-child1"
            alt=""
            src="../assets/image/ellipse-952@2x.png"
          />
          <div class="ethereum-2-parent">
            <img
              class="ethereum-2-icon3"
              alt=""
              src="../assets/image/ethereum-24.svg"
            /><b class="eth2">0.25 ETH</b>
          </div>
          <div class="place-a-bid-wrapper">
            <div class="place-a-bid1"><button>Place a bid</button></div>
          </div>
        </div>
      </div>
      <div class="group-div">
        <div class="unsplash8uzpyniu-rq-parent">
          <img
            class="unsplash8uzpyniu-rq-icon"
            alt=""
            src="../assets/image/unsplashwhj5l9kgtl4@2x.png"
          />
          <div class="arkhan">arkhan</div>
        </div>
        <div class="the-futr-abstr-group">
          <b class="the-futr-abstr1">The Futr Abstr</b>
          <div class="of-8">1 of 8</div>
          <img
            class="group-child1"
            alt=""
            src="../assets/image/ellipse-952@2x.png"
          />
          <div class="ethereum-2-parent">
            <img
              class="ethereum-2-icon3"
              alt=""
              src="../assets/image/ethereum-25.svg"
            /><b class="eth2">0.25 ETH</b>
          </div>
          <div class="place-a-bid-wrapper">
            <div class="place-a-bid1"><button>Place a bid</button></div>
          </div>
        </div>
      </div>
      <b class="top-collections-over">Top Collections over </b
      ><b class="last-7-days">Last 7 days</b>
      <div class="collection-over-child"></div>
      <div class="unsplashk0rvudbob4c-parent">
        <img
          class="unsplashk0rvudbob4c-icon"
          alt=""
          src="../assets/image/unsplashk0rvudbob4c@2x.png"
        /><b class="b">1</b>
        <div class="cryptofunks">CryptoFunks</div>
        <div class="div2">+26.52%</div>
        <div class="parent">
          <div class="div3">19,769.39</div>
          <img
            class="ethereum-1-icon"
            alt=""
            src="../assets/image/ethereum-1.svg"
          />
        </div>
        <div class="verify-1">
          <div class="verify-1-child"></div>
          <img class="vector-icon" alt="" src="../assets/image/vector.svg" />
        </div>
      </div>
      <div class="unsplashft49qnfucq8-parent">
        <img
          class="unsplashft49qnfucq8-icon"
          alt=""
          src="../assets/image/unsplashft49qnfucq8@2x.png"
        /><b class="b1">2</b>
        <div class="cryptix">Cryptix</div>
        <div class="div4">+10.52%</div>
        <div class="group">
          <div class="div3">2,769.39</div>
          <img
            class="ethereum-1-icon"
            alt=""
            src="../assets/image/ethereum-11.svg"
          />
        </div>
      </div>
      <div class="unsplash5mtf9xyvvgm-parent">
        <img
          class="unsplashft49qnfucq8-icon"
          alt=""
          src="../assets/image/unsplash5mtf9xyvvgm@2x.png"
        /><b class="b1">3</b>
        <div class="cryptix">Frensware</div>
        <div class="div6">+2.52%</div>
        <div class="group">
          <div class="div3">9,232.39</div>
          <img
            class="ethereum-1-icon"
            alt=""
            src="../assets/image/ethereum-12.svg"
          />
        </div>
      </div>
      <div class="unsplashzknt5mikuuo-parent">
        <img
          class="unsplashzknt5mikuuo-icon"
          alt=""
          src="../assets/image/unsplashzknt5mikuuo@2x.png"
        /><b class="b">4</b>
        <div class="punkart">PunkArt</div>
        <div class="div8">+1.52%</div>
        <div class="parent1">
          <div class="div3">3,769.39</div>
          <img
            class="ethereum-1-icon"
            alt=""
            src="../assets/image/ethereum-13.svg"
          />
        </div>
        <div class="verify-11">
          <div class="verify-1-child"></div>
          <img class="vector-icon" alt="" src="../assets/image/vector.svg" />
        </div>
      </div>
      <div class="unsplashwjib-6uxa5q-parent">
        <img
          class="unsplashft49qnfucq8-icon"
          alt=""
          src="../assets/image/unsplashwjib6uxa5q@2x.png"
        /><b class="b1">5</b>
        <div class="cryptix">Art Crypto</div>
        <div class="div10">+2.52%</div>
        <div class="parent2">
          <div class="div3">10,769.39</div>
          <img
            class="ethereum-1-icon"
            alt=""
            src="../assets/image/ethereum-14.svg"
          />
        </div>
      </div>
      <div class="collection-over-item"></div>
      <div class="collection-over-inner"></div>
      <div class="line-div"></div>
      <div class="collection-over-child1"></div>
    </div>
    <div class="collection-nft">
      <div class="collection-nft-child"></div>
      <b class="collection-featured-nfts">Collection Featured NFTs</b>
      <div class="unsplashf56y7dgrakc-parent">
        <img
          class="unsplashf56y7dgrakc-icon1"
          alt=""
          src="../assets/image/unsplashf56y7dgrakc1@2x.png"
        /><img
          class="unsplashpvoepplw818-icon1"
          alt=""
          src="../assets/image/unsplashpvoepplw8181@2x.png"
        /><img
          class="unsplashlpbydenbqqg-icon"
          alt=""
          src="../assets/image/unsplashlpbydenbqqg@2x.png"
        /><img
          class="unsplash9anj7qwy-2g-icon"
          alt=""
          src="../assets/image/unsplash9anj7qwy2g@2x.png"
        />
        <div class="amazing-collection-parent">
          <b class="amazing-collection">Amazing Collection</b>
          <div class="by-arkhan-parent">
            <div class="by-arkhan">by Arkhan</div>
            <img
              class="group-child4"
              alt=""
              src="../assets/image/ellipse-955@2x.png"
            />
          </div>
        </div>
        <div class="total-54-items-wrapper">
          <b class="total-54-items">Total 54 Items</b>
        </div>
      </div>
      <div class="unsplashf56y7dgrakc-group">
        <img
          class="unsplashf56y7dgrakc-icon2"
          alt=""
          src="../assets/image/unsplashf56y7dgrakc2@2x.png"
        /><img
          class="unsplashpvoepplw818-icon1"
          alt=""
          src="../assets/image/unsplashpvoepplw8182@2x.png"
        /><img
          class="unsplashlpbydenbqqg-icon1"
          alt=""
          src="../assets/image/unsplashlpbydenbqqg1@2x.png"
        /><img
          class="unsplash9anj7qwy-2g-icon1"
          alt=""
          src="../assets/image/unsplash9anj7qwy2g1@2x.png"
        />
        <div class="amazing-collection-parent">
          <b class="amazing-collection">Amazing Collection</b>
          <div class="by-arkhan-parent">
            <div class="by-arkhan">by Arkhan</div>
            <img
              class="group-child4"
              alt=""
              src="../assets/image/ellipse-955@2x.png"
            />
          </div>
        </div>
        <div class="total-54-items-container">
          <b class="total-54-items">Total 54 Items</b>
        </div>
      </div>
      <div class="unsplashf56y7dgrakc-container">
        <img
          class="unsplashf56y7dgrakc-icon3"
          alt=""
          src="../assets/image/unsplashf56y7dgrakc3@2x.png"
        /><img
          class="unsplashpvoepplw818-icon3"
          alt=""
          src="../assets/image/unsplashpvoepplw8183@2x.png"
        /><img
          class="unsplashlpbydenbqqg-icon2"
          alt=""
          src="../assets/image/unsplashlpbydenbqqg2@2x.png"
        /><img
          class="unsplash9anj7qwy-2g-icon"
          alt=""
          src="../assets/image/unsplash9anj7qwy2g2@2x.png"
        />
        <div class="amazing-collection-parent">
          <b class="amazing-collection">Amazing Collection</b>
          <div class="by-arkhan-parent">
            <div class="by-arkhan">by Arkhan</div>
            <img
              class="group-child4"
              alt=""
              src="../assets/image/ellipse-955@2x.png"
            />
          </div>
        </div>
        <div class="total-54-items-frame">
          <b class="total-54-items">Total 54 Items</b>
        </div>
      </div>
    </div>
    <div class="sign-up-nft">
      <div class="sign-up-now-wrapper">
        <div class="explore-now">
          <button class="text-white">Sign Up Now</button>
        </div>
      </div>
      <img
        class="unsplashtzcrfpsniiq-icon1"
        alt=""
        src="../assets/image/unsplashtzcrfpsniiq1@2x.png"
      /><img
        class="unsplashpvoepplw818-icon4"
        alt=""
        src="../assets/image/unsplashpvoepplw8184@2x.png"
      /><img
        class="unsplashe8ufcyxz514-icon1"
        alt=""
        src="../assets/image/unsplashe8ufcyxz5141@2x.png"
      /><img
        class="sign-up-nft-child"
        alt=""
        src="../assets/image/group-105.svg"
      /><img
        class="sign-up-nft-item"
        alt=""
        src="../assets/image/group-106.svg"
      /><img
        class="sign-up-nft-inner"
        alt=""
        src="../assets/image/group-107.svg"
      /><b class="create-and-sell-your-nfts">Create and sell your NFTs</b>
      <div class="lorem-ipsum-dolor-sit-amet-co2">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Facilisi ac
        phasellus placerat a pellentesque tellus sed egestas. Et tristique
        dictum sit tristique sed non. Lacinia lorem id consectetur pretium diam
        ut. Pellentesque eu sit blandit fringilla risus faucibus.
      </div>
    </div>
    <div class="discover-nft">
      <div class="discover-nft-child"></div>
      <b class="discover-more-nfts">Discover more NFTs</b>
      <div class="content-lisitng-nft">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon"
          alt=""
          src="../assets/image/image-1@2x.png"
        /><b class="kingcrypto">ArtCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-958@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-959@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9510@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9511@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-26.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft1">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon1"
          alt=""
          src="../assets/image/image-11@2x.png"
        /><b class="kingcrypto">ArtCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-958@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-959@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9510@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9511@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-27.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft2">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon"
          alt=""
          src="../assets/image/image-1@2x.png"
        /><b class="kingcrypto">ArtCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-958@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-959@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9510@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9511@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-28.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft3">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-13@2x.png"
        /><b class="kingcrypto">ArtCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-958@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-959@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9510@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9511@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-29.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft4">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-14@2x.png"
        /><b class="kingcrypto">This NFT</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9524@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9525@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9526@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9527@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-210.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft5">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-15@2x.png"
        /><b class="kingcrypto">This NFT</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9524@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9525@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9526@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9527@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-211.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft6">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon1"
          alt=""
          src="../assets/image/image-16@2x.png"
        /><b class="kingcrypto">This NFT</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9524@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9525@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9526@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9527@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-212.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft7">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-14@2x.png"
        /><b class="kingcrypto">This NFT</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9524@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9525@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9526@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9527@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-213.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft8">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-18@2x.png"
        /><b class="kingcrypto">NameCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9540@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9541@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9542@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9543@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-214.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft9">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon"
          alt=""
          src="../assets/image/image-19@2x.png"
        /><b class="kingcrypto">NameCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9540@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9541@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9542@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9543@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-215.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft10">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-110@2x.png"
        /><b class="kingcrypto">NameCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9540@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9541@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9542@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9543@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-216.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft11">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon"
          alt=""
          src="../assets/image/image-19@2x.png"
        /><b class="kingcrypto">NameCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9540@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9541@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9542@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9543@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-217.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft12">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon1"
          alt=""
          src="../assets/image/image-112@2x.png"
        /><b class="kingcrypto">KingCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9556@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9557@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9558@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9559@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-218.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft13">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-113@2x.png"
        /><b class="kingcrypto">KingCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9556@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9557@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9558@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9559@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-219.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft14">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon3"
          alt=""
          src="../assets/image/image-114@2x.png"
        /><b class="kingcrypto">KingCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9556@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9557@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9558@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9559@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-220.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="content-lisitng-nft15">
        <div class="content-lisitng-nft-child"></div>
        <div class="h-50m-2s-left-wrapper">
          <div class="h-50m-2s-left">
            <b>3</b><span>h</span><span class="span"> </span><b>50</b
            ><span>m</span><span class="span"> </span><b>2</b><span>s</span
            ><span class="span"> left</span>
          </div>
        </div>
        <img
          class="image-1-icon1"
          alt=""
          src="../assets/image/image-112@2x.png"
        /><b class="kingcrypto">KingCrypto</b>
        <div class="of-321">1 of 321</div>
        <div class="content-lisitng-nft-item"></div>
        <img
          class="content-lisitng-nft-inner"
          alt=""
          src="../assets/image/ellipse-9556@2x.png"
        /><img
          class="content-lisitng-nft-child1"
          alt=""
          src="../assets/image/ellipse-9557@2x.png"
        /><img
          class="content-lisitng-nft-child2"
          alt=""
          src="../assets/image/ellipse-9558@2x.png"
        /><img
          class="content-lisitng-nft-child3"
          alt=""
          src="../assets/image/ellipse-9559@2x.png"
        />
        <div class="frame-div">
          <img
            class="ethereum-2-icon6"
            alt=""
            src="../assets/image/ethereum-221.svg"
          /><b class="eth2">0.25 ETH</b>
        </div>
        <div class="place-a-bid3"><button>Place a bid</button></div>
      </div>
      <div class="filter">
        <div class="menu">
          <div class="tab-active">
            <div class="all-categories">
              <button class="text-white">All Categories</button>
            </div>
          </div>
          <div class="tab-non-active">
            <div class="all-categories">Art</div>
          </div>
          <div class="tab-non-active">
            <div class="all-categories">Celebrities</div>
          </div>
          <div class="tab-non-active">
            <div class="all-categories">Gaming</div>
          </div>
          <div class="tab-non-active">
            <div class="all-categories">Sport</div>
          </div>
          <div class="tab-non-active">
            <div class="all-categories">Music</div>
          </div>
          <div class="tab-non-active">
            <div class="all-categories">Crypto</div>
          </div>
        </div>
        <div class="filter1">
          <img
            class="filter-results-button-1-icon"
            alt=""
            src="../assets/image/filterresultsbutton-1.svg"
          />
          <div class="all-categories">All Filters</div>
        </div>
      </div>
      <div class="cta-more-nft">
        <div class="more-nfts">
          <button>More NFTs</button>
        </div>
      </div>
    </div>
    <div class="footer">
      <div class="marketplace1">
        <div class="all-nfts-new-art-sports-utilit">
          <p class="all-nfts"><a href="#">All NFTs</a></p>
          <p class="all-nfts"><a href="#">New</a></p>
          <p class="all-nfts"><a href="#">Art</a></p>
          <p class="all-nfts"><a href="#">Sports</a></p>
          <p class="all-nfts"><a href="#">Utility</a></p>
          <p class="all-nfts"><a href="#">Music</a></p>
          <p class="domain-name"><a href="#">Domain Name</a></p>
        </div>
        <b class="market-place"><a href="#">Market Place</a></b>
      </div>
      <div class="nft-marketplace">
        <b class="nfters1">NFters </b>
        <div class="the-worlds-first-and-largest">
          The world’s first and largest digital marketplace for crypto
          collectibles and non-fungible tokens (NFTs). Buy, sell, and discover
          exclusive digital items.
        </div>
        <button>
          <img
            class="facebook-icon"
            alt=""
            src="../assets/image/facebook.svg"
          />
        </button>
        <button>
          <img class="twitter-icon" alt="" src="../assets/image/twitter.svg" />
        </button>
        <button>
          <img
            class="linkedin-icon"
            alt=""
            src="../assets/image/linkedin.svg"
          />
        </button>
      </div>
      <div class="my-account">
        <div class="all-nfts-new-art-sports-utilit">
          <p class="all-nfts"><a href="#">Profile</a></p>
          <p class="all-nfts"><a href="#">Favorite</a></p>
          <p class="all-nfts"><a href="#">My Collections</a></p>
          <p class="all-nfts"><a href="#">Settings</a></p>
        </div>
        <b class="my-account1"><a href="#">My Account</a></b>
      </div>
      <div class="stay-in-the-loop">
        <div class="join-our-mailing-list-to-stay">
          Join our mailing list to stay in the loop with our newest feature
          releases, NFT drops, and tips and tricks for navigating NFTs.
        </div>
        <b class="stay-in-the-loop1">Stay in the loop</b>
        <div class="field-email">
          <div class="field-email-child"></div>
          <input
            type="email"
            placeholder="Enter your email address.."
            class="enter-your-email-address"
          />
          <div class="cta">
            <button class="subscibe-now">
              <button class="text-white">Subscibe Now</button>
            </button>
          </div>
        </div>
      </div>
      <div class="copyright">
        <div class="copyright-2022-avi-yansah">Copyright © 2022 Avi Yansah</div>
        <div class="copyright-child"></div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "HelloWorld",
  props: {
    msg: String,
  },
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
h3 {
  margin: 40px 0 0;
}
ul {
  list-style-type: none;
  padding: 0;
}
li {
  display: inline-block;
  margin: 0 10px;
}
input {
  outline-offset: 0ch;
  border: 0px;
}
input:focus {
  outline: none;
}
button {
  outline: none;
  background-color: inherit;
  border: 0;
}
a {
  list-style-type: none;
  color: inherit;
}

a:hover {
  color: indigo;
}
</style>
